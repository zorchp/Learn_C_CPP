...
public:
    ...
    // 克隆自身
    virtual OrderApi* cloneOrder() = 0; // refactor
...
public:
    ...
    // add this :
    OrderApi* cloneOrder() {
        AbroadOrder* pAbroadOrder = new AbroadOrder;
        pAbroadOrder->setCustomerName(m_strCustomerName);
        pAbroadOrder->setProductId(m_strProductId);
        pAbroadOrder->setOrderProductNum(m_orderProductNum);
        return pAbroadOrder;
    }
...
public:
    ...
    void saveOrder(OrderApi* pOrder) {
        while (pOrder->getOrderProductNum() > 200) {
            // 新建订单
            OrderApi* pNewOrder = pOrder->cloneOrder(); // 拷贝子类的特有内容
